...
export default function Home() {
  ...
  const [showCompetitionReset, setShowCompetitionReset] = useState(false);
  const [competitionName, setCompetitionName] = useState("GOSUP GAMES");
  const [competitionStartDate, setCompetitionStartDate] = useState("2026-08-01");
  const [competitionEndDate, setCompetitionEndDate] = useState("2026-08-31");
  const [competitionResetConfirmation, setCompetitionResetConfirmation] = useState("");
  ...
  function openCompetitionReset() {
    setCompetitionName(me?.competition.name || "GOSUP GAMES");
    setCompetitionStartDate(me?.competition.startDate || "2026-08-01");
    setCompetitionEndDate(me?.competition.endDate || "2026-08-31");
    setCompetitionResetConfirmation("");
    setShowCompetitionReset(true);
  }
    ...
    try {
      await api("/api/admin/reset-contest", {
        ...
        body: JSON.stringify({
          ...
          timezone: me?.competition.timezone || "Europe/Moscow",
          reminderTime,
          confirmation: competitionResetConfirmation,
          comment: "Новый конкурс создан через административный интерфейс",
        }),
      });
      setShowCompetitionReset(false);
      setCompetitionResetConfirmation("");
      setTab("today");
      notify("Все пользователи удалены. Для участия им нужно зарегистрироваться заново");
      await load();
    } catch (caught) {
      notify(caught instanceof Error ? caught.message : "Не удалось начать конкурс заново");
    }
  }
  ...
  return <main className="app-shell">
    ...
    {showCompetitionReset && <div className="modal-backdrop"><section className="modal"><button className="modal-close" onClick={() => setShowCompetitionReset(false)}>×</button><span className="eyebrow">Полный сброс</span><h2>Удалить пользователей и начать заново</h2><p>Все пользователи и связанные с ними результаты будут удалены без возможности восстановления. Каждый участник, включая администратора, должен будет зарегистрироваться заново.</p><label className="note-field">Название конкурса<input maxLength={160} value={competitionName} onChange={(event) => setCompetitionName(event.target.value)}/></label><label className="note-field">Дата начала<input type="date" value={competitionStartDate} onChange={(event) => setCompetitionStartDate(event.target.value)}/></label><label className="note-field">Дата окончания<input type="date" value={competitionEndDate} onChange={(event) => setCompetitionEndDate(event.target.value)}/></label><label className="note-field">Для подтверждения введите «УДАЛИТЬ ВСЕХ ПОЛЬЗОВАТЕЛЕЙ»<input value={competitionResetConfirmation} onChange={(event) => setCompetitionResetConfirmation(event.target.value)}/></label><button className="primary danger" disabled={competitionResetConfirmation !== "УДАЛИТЬ ВСЕХ ПОЛЬЗОВАТЕЛЕЙ" || !competitionName.trim() || !competitionStartDate || !competitionEndDate} onClick={() => void resetCompetition()}>Удалить всех и начать заново</button></section></div>}
