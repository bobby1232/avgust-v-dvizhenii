...
export default function Home() {
  ...
  async function resetCompetition() {
    try {
      ...
      setCompetitionResetConfirmation("");
      setTab("today");
      notify("Новый конкурс создан, результаты начаты заново");
      await load();
    } catch (caught) {
  ...
  return <main className="app-shell">
    ...
    {showCompetitionReset && <div className="modal-backdrop"><section className="modal"><button className="modal-close" onClick={() => setShowCompetitionReset(false)}>×</button><span className="eyebrow">Новый этап</span><h2>Сбросить результаты и начать заново</h2><p>Мы не удалим текущие данные: завершённый конкурс останется в истории. Новые отметки и результаты будут собираться отдельно.</p><label className="note-field">Название конкурса<input maxLength={160} value={competitionName} onChange={(event) => setCompetitionName(event.target.value)}/></label><label className="note-field">Дата начала<input type="date" value={competitionStartDate} onChange={(event) => setCompetitionStartDate(event.target.value)}/></label><label className="note-field">Дата окончания<input type="date" value={competitionEndDate} onChange={(event) => setCompetitionEndDate(event.target.value)}/></label><label className="note-field">Для подтверждения введите «НАЧАТЬ ЗАНОВО»<input value={competitionResetConfirmation} onChange={(event) => setCompetitionResetConfirmation(event.target.value)}/></label><button className="primary danger" disabled={competitionResetConfirmation !== "НАЧАТЬ ЗАНОВО" || !competitionName.trim() || !competitionStartDate || !competitionEndDate} onClick={() => void resetCompetition()}>Сбросить результаты и начать заново</button></section></div>}
