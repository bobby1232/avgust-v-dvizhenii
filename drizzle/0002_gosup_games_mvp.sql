...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'activities_duration_check'
      AND conrelid = 'activities'::regclass
  ) THEN
    ALTER TABLE "activities"
      ADD CONSTRAINT "activities_duration_check"
      CHECK ("duration_minutes" BETWEEN 20 AND 1440);
  END IF;
...
CREATE UNIQUE INDEX IF NOT EXISTS "activity_days_competition_user_date_idx"
  ON "activity_days" ("competition_id", "user_id", "activity_date");
CREATE INDEX IF NOT EXISTS "activity_days_user_idx" ON "activity_days" ("user_id");
...
CREATE TABLE IF NOT EXISTS "achievement_definitions" (
  ...
  "category" varchar(64) DEFAULT 'streak' NOT NULL,
...
INSERT INTO "achievement_definitions"
  ("code", "name", "description", "emoji", "trigger_type", "trigger_value")
VALUES
...
CREATE TABLE IF NOT EXISTS "user_achievements" (
  ...
  "source" varchar(32) DEFAULT 'automatic' NOT NULL,
  "revoked_at" timestamptz,
  "revoked_by" bigint REFERENCES "users"("id"),
  "comment" varchar(500)
);
CREATE UNIQUE INDEX IF NOT EXISTS "user_achievements_unique_idx"
  ON "user_achievements" ("competition_id", "user_id", "achievement_id");
...
CREATE UNIQUE INDEX IF NOT EXISTS "notification_logs_unique_idx"
  ON "notification_logs" ("competition_id", "user_id", "notification_type", "notification_date");

CREATE TABLE IF NOT EXISTS "contest_settings" (
  "id" bigserial PRIMARY KEY,
  "competition_id" bigint NOT NULL UNIQUE REFERENCES "competitions"("id") ON DELETE CASCADE,
  "minimum_duration_minutes" integer DEFAULT 20 NOT NULL,
...
SELECT competition."id", theme.start_date::date, theme.end_date::date, theme.title
FROM "competitions" AS competition
CROSS JOIN (VALUES
  ('2026-08-01', '2026-08-02', 'Начинаем'),
  ('2026-08-03', '2026-08-09', 'Пробуем новое'),
  ('2026-08-10', '2026-08-16', 'Двигаемся вместе'),
  ('2026-08-17', '2026-08-23', 'Не останавливаемся'),
  ('2026-08-24', '2026-08-31', 'Финишируем')
) AS theme(start_date, end_date, title)
WHERE competition."is_active" = true
  AND NOT EXISTS (
    SELECT 1
    FROM "weekly_themes" AS existing
    WHERE existing."competition_id" = competition."id"
      AND existing."start_date" = theme.start_date::date
      AND existing."end_date" = theme.end_date::date
  );
...
CREATE UNIQUE INDEX IF NOT EXISTS "weekly_reports_unique_idx"
  ON "weekly_reports" ("competition_id", "report_date");
...
CREATE TABLE IF NOT EXISTS "draws" (
  ...
  "exclude_previous_winners" boolean DEFAULT false NOT NULL,
  "comment" varchar(500),
...
CREATE UNIQUE INDEX IF NOT EXISTS "draw_participants_unique_idx"
  ON "draw_participants" ("draw_id", "user_id");
...
CREATE UNIQUE INDEX IF NOT EXISTS "draw_winners_user_idx"
  ON "draw_winners" ("draw_id", "user_id");
CREATE UNIQUE INDEX IF NOT EXISTS "draw_winners_position_idx"
