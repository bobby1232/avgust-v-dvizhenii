...
import { achievementCardDataUrl } from "./achievement-card";
...
async function claimEvents(competitionId: number, digestIntervalMinutes: number,
  publishEachActivityEnabled: boolean): Promise<Event[]> {
  ...
  try {
    ...
    return result.rows.map((row: any) => ({
      ...row,
      competitionId: Number(row.competition_id), eventType: row.event_type, entityType: row.entity_type,
      ...
      lastAttemptAt: row.last_attempt_at, errorMessage: row.error_message,
...
async function finish(events: Event[], status: "sent" | "failed" | "skipped",
  messageId?: number, error?: unknown) {
...
async function schedule(competition: Awaited<ReturnType<typeof activeCompetition>>,
  settings: typeof contestSettings.$inferSelect, now: Date) {
  ...
  for (const [slot, due] of [
    ...
    if (settings.leaderboardAnnouncementsEnabled && time >= due.slice(0, 5)) values.push({
      competitionId: competition.id, eventType: "leaderboard", entityType: "competition",
      entityId: String(competition.id), dedupeKey: `leaderboard:${competition.id}:${date}:${slot}`,
      payload: { competitionDate: date, slot },
    });
  }
  if (settings.dailySummaryEnabled && time >= publicationTimes.dailySummaryTime) values.push({
    competitionId: competition.id, eventType: "daily-summary", entityType: "competition",
    entityId: String(competition.id), dedupeKey: `daily-summary:${competition.id}:${date}:evening`,
...
async function audit(action: string, competitionId: number, events: Event[],
  publicationType: string, messageId?: number, result = "sent") {
  await db.insert(auditLogs).values({
    action, entityType: "group_feed", entityId: events.map((event) => event.id).join(","), result,
    payload: {
      competitionId, eventIds: events.map((event) => event.id), entityIds: events.map((event) => event.entityId),
      publishedCount: events.length, telegramMessageId: messageId ? String(messageId) : undefined,
...
function activityText(events: Event[], todayActive: number, total: number, todayActivities: number) {
  const lines = events.map((event) => {
    ...
    return `🏃 ${participantName(p.displayName, p.telegramUsername)}${dept} — ${h(name)}, ${p.durationMinutes} мин.${detail}`;
  });
  return ["🔥 <b>Новые активности в GOSUP GAMES</b>", "", ...lines, "",
    ...
    "Открыть приложение: @Gosup_comp_bot"].join("\n");
}
...
function singleActivityText(event: Event) {
  ...
  const dept = p.department ? ` (${h(p.department)})` : "";
  return ["🏃 <b>Новая активность в GOSUP GAMES</b>", "",
    `${participantName(p.displayName, p.telegramUsername)}${dept}`,
    `<b>${h(name)}</b> · ${p.durationMinutes} мин.${detail}`, "",
    "Открыть приложение: @Gosup_comp_bot"].join("\n");
}
...
export async function processGroupFeed(now = new Date()) {
  ...
  let settings = stored;
  if (!settings) {
    [settings] = await db.insert(contestSettings).values({ competitionId: competition.id })
      .onConflictDoNothing().returning();
    if (!settings) [settings] = await db.select().from(contestSettings)
      .where(eq(contestSettings.competitionId, competition.id)).limit(1);
  }
  const chatId = settings?.reportChatId ?? process.env.TELEGRAM_REPORT_CHAT_ID;
  if (!chatId) return { sent: 0, skipped: 0, failed: 0, disabled: true,
    reason: "GROUP_CHAT_NOT_CONFIGURED" as const };
  if (!settings.groupFeedEnabled) return { sent: 0, skipped: 0, failed: 0, disabled: true,
    reason: "GROUP_FEED_DISABLED" as const };
  ...
  const result = {
    activityPostsSent: 0, activityDigestsSent: 0, activitiesPublished: 0,
    achievementsPublished: 0, leaderboardsPublished: 0, dailySummariesPublished: 0,
    skipped: 0, failed: 0,
  };
  ...
  const dayActivities = await db.select().from(activities).where(and(
    eq(activities.competitionId, competition.id), eq(activities.activityDate, today),
    eq(activities.status, "approved"),
  ));
  const todayDays = await db.select().from(activityDays).where(and(
    eq(activityDays.competitionId, competition.id), eq(activityDays.activityDate, today),
  ));
  ...
  const individualEvents = settings.publishEachActivityEnabled
    ? activityEvents : activityEvents.filter((event) => eventPhotos(event).length > 0);
  ...
  for (const event of individualEvents) {
    try {
      ...
      result.activityPostsSent++;
      result.activitiesPublished++;
    } catch (error) {
  ...
  if (!settings.activityDigestEnabled) {
    ...
    for (let offset = 0; offset < digestEvents.length; offset += 15) {
      ...
      try {
        const sent = await sendLongTelegramMessage(chatId,
          activityText(batch, todayDays.length, community.registeredParticipants, dayActivities.length));
        const id = sent.at(-1)!.message_id;
        await finish(batch, "sent", id);
        await audit("GROUP_ACTIVITY_DIGEST_SENT", competition.id, batch, "activity_digest", id);
        result.activityDigestsSent++;
  ...
  for (const event of events.filter((item) => item.eventType === "achievement")) {
    ...
    const [award] = await db.select({ row: userAchievements, user: users, definition: achievementDefinitions })
      ...
      .where(and(eq(userAchievements.id, Number(event.entityId)), isNull(userAchievements.revokedAt)))
      .limit(1);
    if (!award) {
      await finish([event], "skipped");
      result.skipped++;
    ...
    const text = [manual ? "🎖 <b>Специальный бейдж</b>" : "🏅 <b>Новое достижение!</b>", "",
      ...
      "Поздравим реакциями! 👏", "", "Открыть приложение: @Gosup_comp_bot"].join("\n");
    try {
      const card = await achievementCardDataUrl(award.definition.code);
      let id: number;
      if (card) {
        id = (await sendTelegramPhotoPost(chatId, [card], text)).message_id;
      } else {
        id = (await sendLongTelegramMessage(chatId, text)).at(-1)!.message_id;
      }
      await finish([event], "sent", id);
      await audit("GROUP_ACHIEVEMENT_SENT", competition.id, [event], "achievement", id);
      result.achievementsPublished++;
    } catch (error) {
      await finish([event], "failed", undefined, error);
  ...
  for (const event of events.filter((item) =>
    ...
    if (event.eventType === "leaderboard") {
      ...
      const remaining = competition.endDate
        ...
        : "—";
      text = ["🏆 <b>Лидеры GOSUP GAMES — " + h(today) + "</b>", "", ...top, "",
        `Сегодня отметились: <b>${todayDays.length} участников</b>`,
        `В игре: <b>${community.registeredParticipants} участник</b>`,
        `До завершения игры: <b>${remaining} дней</b>`, "", "Продолжаем движение! 💪", "",
        "Открыть таблицу: @Gosup_comp_bot"].join("\n");
      action = "GROUP_LEADERBOARD_SENT";
    } else {
      const achievementsToday = await db.select().from(userAchievements).where(and(
        eq(userAchievements.competitionId, competition.id), isNull(userAchievements.revokedAt),
      ...
      text = ["🌙 <b>Итоги дня GOSUP GAMES</b>", "",
        ...
        popular ? `Самая популярная активность сегодня — ${h(popular)}.` : "Сегодня ещё можно успеть добавить активность.",
        "", "Завтра продолжаем. Главное — не останавливаться.", "",
        "Продолжить в приложении: @Gosup_comp_bot"].join("\n");
    ...
    try {
      ...
      const id = sent.at(-1)!.message_id;
      await finish([event], "sent", id);
      await audit(action, competition.id, [event], event.eventType, id);
      if (event.eventType === "leaderboard") result.leaderboardsPublished++;
      else result.dailySummariesPublished++;
    } catch (error) {
      await finish([event], "failed", undefined, error);
